        ...
        <div className="p-4 border-b border-slate-200 flex flex-col sm:flex-row items-center justify-between gap-4 bg-slate-50/50">
          <div className="relative w-full sm:w-72">
            ...
            <input type="text" placeholder="Search files..."
              ...
              className="w-full pl-9 pr-4 py-2 text-sm border border-slate-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500 bg-white" />
          </div>
          <button className="flex items-center text-slate-600 px-3 py-2 border border-slate-300 rounded-lg hover:bg-slate-50 text-sm font-medium">
                ...
                filteredFiles.map((file) => (
                  <tr key={file.id} className="hover:bg-slate-50/50 transition-colors">
                    ...
                    <td className="px-6 py-4">
                      ...
                      {file.expiry !== 'N/A' && <p className="text-xs text-slate-400 mt-1">Exp: {file.expiry}</p>}
                    </td>
                    <td className="px-6 py-4 text-right space-x-2">
                      <button onClick={() => handleCopyLink(file.token)}
                        className="p-2 text-slate-400 hover:text-indigo-600 hover:bg-slate-100 rounded-lg transition-colors" title="Copy Link">
                        <Copy className="w-4 h-4" />
                      </button>
                      <Link to="/analytics"
                        className="inline-flex p-2 text-slate-400 hover:text-indigo-600 hover:bg-slate-100 rounded-lg transition-colors" title="Analytics">
