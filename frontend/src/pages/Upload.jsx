...
const generatedLink = `${window.location.origin}/file/${res.data.data.token}`;
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
        {uploadSuccess ? (
          <div className="p-8 pb-10 flex flex-col items-center justify-center text-center">
            ...
            <p className="text-slate-600 mb-8 max-w-md">Your file <span className="font-semibold">{file.name}</span> has been securely uploaded and is ready to share.</p>
            ...
            <div className="w-full max-w-md bg-slate-50 border border-slate-200 rounded-xl p-4 flex flex-col sm:flex-row items-center gap-3">
              <input 
                ...
                readOnly 
                value={shareLink} 
                className="flex-1 w-full bg-white border border-slate-300 rounded-lg py-2 px-3 text-sm text-slate-600 focus:outline-none focus:ring-2 focus:ring-indigo-500"
              />
              <button 
            ...
            {!file ? (
              ...
              <div className="space-y-8 mt-2">
                <div className="flex items-center justify-between p-4 bg-slate-50 border border-slate-200 rounded-xl relative overflow-hidden">
                  ...
                  {!uploading && (
                    <button onClick={clearFile} className="p-2 text-slate-400 hover:text-red-500 hover:bg-red-50 rounded-lg transition-colors flex-shrink-0">
                      <X className="w-5 h-5" />
                    </button>
                  )}
                </div>
                ...
                <div className="bg-slate-50 rounded-xl p-6 border border-slate-200">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                      ...
                      <select 
                        className="w-full text-sm border-slate-300 rounded-lg py-2 pl-3 pr-8 focus:ring-indigo-500 focus:border-indigo-500 border bg-white"
                    ...
                    <div>
                      ...
                      <input 
                        type="number" 
                        placeholder="e.g. 10" 
                        className="w-full text-sm border-slate-300 rounded-lg py-2 px-3 focus:ring-indigo-500 focus:border-indigo-500 border bg-white"
