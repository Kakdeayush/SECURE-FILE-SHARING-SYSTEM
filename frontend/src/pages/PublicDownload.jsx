...
const PublicDownload = () => {
  ...
  useEffect(() => {
    const fetchFileInfo = async () => {
      try {
        const res = await api.get(`/public/info/${token}`);
        ...
        setError('This link is invalid, expired, or the download limit has been reached.');
      } finally {
        setLoading(false);
      }
    };
  ...
  const handlePasswordSubmit = async (e) => {
    ...
    setPasswordError('');
    try {
      const res = await api.post(`/public/verify-password/${token}`, { password });
  ...
  const handleDownload = async () => {
    setDownloading(true);
    try {
      const url = `http://localhost:8080/api/public/download/${token}`;
      const link = document.createElement('a');
      link.href = password ? `${url}?password=${password}` : url;
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-indigo-50 to-slate-50 flex flex-col justify-center py-12 sm:px-6 lg:px-8 font-inter">
      ...
      <div className="sm:mx-auto sm:w-full sm:max-w-md px-4">
          ...
          {requiresPassword ? (
            <form onSubmit={handlePasswordSubmit}>
              <div className="text-center mb-6">
                ...
                <p className="text-sm text-slate-500 mt-2">This file is protected. Please enter the password to access it.</p>
              </div>
            ...
            <div>
              ...
              <div className="bg-slate-50 border border-slate-200 rounded-xl p-5 mb-8">
                <div className="flex items-start">
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="font-semibold text-slate-800 truncate mb-1" title={fileData.name}>
                      {fileData.name}
                    </p>
              ...
              <button
                ...
                className="w-full flex items-center justify-center py-3.5 px-4 border border-transparent rounded-lg shadow-[0_4px_14px_0_rgba(79,70,229,0.39)] text-base font-semibold text-white bg-indigo-600 hover:bg-indigo-700 hover:shadow-[0_6px_20px_rgba(79,70,229,0.23)] disabled:opacity-70 disabled:shadow-none transition-all"
              >
                {downloading ? (
                  <>
        ...
        <p className="text-center text-xs text-slate-400 mt-8">
          Report abuse or suspicious files
        </p>
      </div>
    </div>
  );
